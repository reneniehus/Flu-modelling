...
if (T) { # 4 mins
  tic()
  for (NE_and_dark_factor_i in seq(from=log(20),to=log(200),length.out=30) ) {
    for ( wane_imm_fract_weekly_1_log_i in seq(from=logit(0.20),to=logit(0.20),length.out=1) ) {
      for ( Imm_2_partial_i in seq(from=logit(0.1),to=logit(0.5),length.out=40) ) {
        for ( wane_imm_fract_weekly_2_log_i in seq(from=logit(0.01),to=logit(0.01),length.out=1) ) {
...
df_Rt_without_imm$Rt_without_immunity

```

### *Fit augmented Rt*

```{r eval=FALSE, fig.height=10, fig.width=10, message=TRUE, include=FALSE}
