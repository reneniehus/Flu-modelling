...
if (T) {
  tic()
  for (NE_and_dark_factor_i in seq(from=25,to=50,length.out=8) ) {
    for ( wane_imm_fract_weekly_1_log_i in seq(from=-8,to=-0.2,length.out=8) ) {
      for ( Imm_2_partial_i in seq(from=0.3,to=0.9,length.out=8) ) {
        for ( wane_imm_fract_weekly_2_log_i in seq(from=-18,to=-8,length.out=8) ) {
