...
for (i_trans in tranv_v) { # run same model for diff scaling # i_trans = tranv_v[1]
  ...
  for (country_i in country_v) { # country_i = country_v[1]
    ...
    # ---- |-Fit stan model ----
    if (mytransformation=="sqrt") myfile=paste0("../Big data/respicasting_",country_i,"sqrt_fit01_death.Rdata" )
    if (mytransformation=="log") myfile=paste0("../Big data/respicasting_",country_i,"log_fit01_death.Rdata" )
  ...
  if (mytransformation=="log") write_csv( respicast_df , file=paste0("./output/covid-forecast-hub/ECDC-norrsken_green/",myorigin,"-ECDC-norrsken_green_death.csv") )
  if (mytransformation=="sqrt") write_csv( respicast_df , file=paste0("./output/covid-forecast-hub/ECDC-norrsken_blue/",myorigin,"-ECDC-norrsken_blue_death.csv") )
...
if (F){
  ...
  library(hubVis)
  mod_log = read_csv(file=paste0("./output/covid-forecast-hub/ECDC-norrsken_green/",myorigin,"-ECDC-norrsken_green_death.csv"))
  mod_sqrt = read_csv(file=paste0("./output/covid-forecast-hub/ECDC-norrsken_blue/",myorigin,"-ECDC-norrsken_blue_death.csv"))
