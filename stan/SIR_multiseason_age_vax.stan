...
generated quantities {
  ...
  matrix<lower=0>[n_day_project, n_age_groups, n_scenario] gen_delta_severe;
  matrix<lower=0>[n_day_project, n_age_groups, n_scenario] gen_severe_mean;
  array[n_week_fit, n_age_groups] int<lower=0> gen_severe_obs_fit;
  array[n_week_project, n_age_groups, n_scenario] int<lower=0> gen_severe_obs_project;
  ...
  for (j in 1:n_scenario) {
    ...
    if ( axis_transmission[j]=="pess" ) beta_j = 1.1*beta; // more var: 1.1*normal_rng( log2( beta ), 0.25 ) // pessimistic transmission
    if ( axis_transmission[j]=="opti" ) beta_j = 0.9*beta; // more var: 0.9*normal_rng( log2( beta ), 0.25 ) // optimisitc transmission
    if ( axis_vax[j]=="opti" ) delta_vax_j = delta_vax_opti;
    if ( axis_vax[j]=="pess" ) delta_vax_j = delta_vax_pess;
    if ( axis_vax[j]=="null" ) delta_vax_j = delta_vax_null; 
    ...
    for (t in 1:n_day_project) {
      for (a in 1:n_age_groups) {
        ...
        //
        gen_delta_severe[t,a,j] = (delta_infective_exposures_u + delta_infective_exposures_v * (1-ve_severe)) * prop_severe_mu[a]; 
        gen_severe_mean[t,a,j] = gen_delta_severe[t,a,j] * pop_age_group[a,1] ;
        //
        if (t==2) { // also impute the first position
        gen_severe_mean[1,a,j] = gen_severe_mean[2,a,j];
        gen_delta_severe[1,a,j] = gen_delta_severe[2,a,j];
        }
        }
      } // n_age_groups loop 
    } // n_day_project loop
  ...
  // convert projections daily to weekly
  for (j in 1:n_scenario) {
    for (i in 1:n_week_project) {
      for (a in 1:n_age_groups) {
        ...
        int day_end = day_start+6;
        gen_severe_mean_weekly[i,a,j] = sum( gen_severe_mean[day_start:day_end,a,j] );
      }
    }
  }
  ...
  for (j in 1:n_scenario) {
    for (t in 1:n_week_project) {
      for (a in 1:n_age_groups) {
        gen_severe_obs_project[t,a,j] = neg_binomial_2_rng( gen_severe_mean_weekly[t,a,j] , phi ) ;
      }
    }
  }
